...
| **APK (rebuild — React+TS/Capacitor)** | `APK/mobile/` | 🟡 Employee + HR-admin/owner flows real and verified (web layer) | Started 2026-09-01. Vite+React+TS+Tailwind (tokens mirrored from `DESIGN_SYSTEM_GUIDE.md`), wired to the real `EDGE/backend/routes/apk.js` contract. `npm install`, `tsc -b`, `vite build` all pass; `npx cap sync android` auto-registers all **8** reused/new plugins. Screens done: server-address setup, login, home/today-status, full attendance-marking flow (liveness+face capture → match → GPS → submit), and the full HR-admin/owner suite (live feed, employees, work assignments, alert subscriptions, analytics, broadcast) under `src/pages/admin/`, role-gated to match the backend's own permission split. **Not done:** attendance history, announcements feed, face-enrollment capture UI, profile/settings, FCM wiring, offline batch-sync. No Gradle/on-device build attempted yet — this dev environment has no Android SDK; also needs the `mobilefacenet.tflite` model file dropped in before it can build (see next row). |
| **`@jenix/cap-face-liveness` (new plugin)** | `APK/mobile/native-plugins/cap-face-liveness/` | 🟡 Ported, TS side tested, native side unverified | New Capacitor plugin, built 2026-09-01, **EdgeFolio-only — deliberately not placed in `EMS/Plugins`** (that workspace is FieldForce-only; see §4). Native Kotlin (`LivenessDetector.kt`, `CameraXManager.kt`, `FaceEmbeddingEngine.kt`) ported near-verbatim from `APK/android`'s already-validated originals — same 2-blinks/5°-head-turn liveness thresholds, same MobileFaceNet TFLite contract. TS side (`similarity.ts` — cosine match, done in JS since it's cheap) builds and passes tests. **Not verified:** an actual Gradle compile of the Kotlin — no Android SDK here. Needs `mobilefacenet.tflite` placed at `APK/mobile/android/app/src/main/assets/` before any build will succeed (same requirement `APK/android` already had, never committed — binary size). Simplification vs. native: no face-bounding-box overlay, hint text only — see the plugin's own `README.md`. |
...
- **Still backlog, not built:** attendance history (employee), announcements feed
  ...
  offline batch-sync wiring. None of these block a first real device test of what
  exists — that's still the next concrete step (Gradle build + `mobilefacenet.tflite`
  + physical device).
